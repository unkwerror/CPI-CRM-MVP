...
3. **`participant-hygiene --apply` запускается при каждом деплое** и архивирует
   всех, у кого `canonical_full_name` не разбирается в русское ФИО. Личности,
   заведённые Telegram-ботом по имени из профиля, после деплоя пропадают из
   реестра участников — это ожидаемое поведение, чинится слиянием карточек.

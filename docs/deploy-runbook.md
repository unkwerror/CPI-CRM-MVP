...
4. **`participant-hygiene --apply` запускается при каждом деплое** и оставляет в
   рабочем реестре только карточки со строгим русским ФИО. Бот автоматически создаёт
   карточку только при полном пригодном ФИО; неполные и неоднозначные отправки остаются
   в очереди «Заявки из бота» до ручной привязки и не теряются.

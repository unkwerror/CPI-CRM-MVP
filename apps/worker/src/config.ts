...
export interface WorkerConfig {
  ...
  readonly campaigns: {
    ...
    /** Пустой ключ выключает email-канал так же, как пустой токен — Telegram. */
    readonly unisenderApiKey: string;
    readonly unisenderApiUrl: string;
    /** Предохранитель от случайной отправки всей базы за один день. */
...
export function loadConfig(): WorkerConfig {
  return {
    ...
    campaigns: {
      ...
      unisenderApiKey: process.env.UNISENDER_API_KEY?.trim() ?? '',
      unisenderApiUrl: value(
        'UNISENDER_API_URL',

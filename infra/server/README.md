...
Для интеграции с Locker задайте `LOCKER_BASE_URL` и случайный
`LOCKER_INTEGRATION_TOKEN` длиной не менее 32 символов. Точно тот же токен должен
находиться в `/opt/CPI-TG-BOT/infra/server/.env`; генерировать его удобно командой
`openssl rand -hex 32`. Это серверный секрет, его нельзя передавать браузеру или боту.
